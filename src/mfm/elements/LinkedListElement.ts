...
export class LinkedList extends Element {
  static BASE_TYPE: IElementType = { name: "LINKEDLISTELEMENT", symbol: "Ll", class: LinkedList, color: 0xcc0066 };
  ...
  moveTo(ew: EventWindow, relativeIndexToGoTo: number, leavingAtom?: Atom, maxIndex: number = 8): boolean {
    ...
    if (goSite && goSite.atom.is(Empty.BASE_TYPE)) {
  ...
  exec(ew: EventWindow) {
    ...
    if (this.linkType !== "INITIAL" && !this.hasMoved(ew) && !this.next && !this.prev) {
      if (ew.origin.atom.data) {
        ew.origin.atom = Data.CREATE({ params: [ew.origin.atom.data.value] });
      } else {
        ew.origin.die();
      }
    }
