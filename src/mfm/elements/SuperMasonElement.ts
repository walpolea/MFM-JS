...
export class SuperMason extends Element {
  static BASE_TYPE: IElementType = { name: "SUPERMASON", symbol: "SMa", class: SuperMason, color: 0x20cccc };
  static CREATE = SuperMason.CREATOR();
  static RANDOM_CREATE = SuperMason.CREATOR({ params: [SuperMason.randomPath(20)] });
  ...
  constructor(_path: string = undefined, _curIndex: number = 0) {
    super(SuperMason.BASE_TYPE, 100, 100);
  ...
  exec(ew: EventWindow) {
    ...
    if (lastdir !== dir) {
      ...
      if (lastOuterBuildSite) {
        ew.origin.mutateSite(lastOuterBuildSite, MembraneWall.CREATE({ params: [0.1, EventWindow.LAYER4, Data.BASE_TYPE] }));
      }
    }

    //build the outer wall
    if (outerBuildSite) {
      if (outerBuildSite.atom.is(Res.BASE_TYPE) || ew.is(outerBuildSite, [Empty.BASE_TYPE, StuckMembrane.BASE_TYPE])) {
        ew.origin.mutateSite(outerBuildSite, MembraneWall.CREATE({ params: [0.1, EventWindow.LAYER4, Data.BASE_TYPE] }));
      }
    }

    //build the inner wall
    if (innerBuildSite) {
      if (innerBuildSite.atom.is(Res.BASE_TYPE) || ew.is(innerBuildSite, [Empty.BASE_TYPE, StuckMembrane.BASE_TYPE])) {
        ew.origin.mutateSite(innerBuildSite, MembraneWall.CREATE({ params: [0.1, EventWindow.ADJACENT8WAY, Data.BASE_TYPE] }));
      }
    }

    //move to next site and leave another mason to help
    if (moveSite) {
      ew.origin.mutateSite(moveSite, SuperMason.CREATE({ params: [SuperMason.pathToString(this.path), this.curIndex + 1] }));
    }

    if (lastSite) {
      ew.origin.mutateSite(lastSite, SuperMason.CREATE({ params: [SuperMason.pathToString(this.path), this.curIndex - 1] }));
    }
...
ElementRegistry.registerType(SuperMason.BASE_TYPE);
