...
export class Mason extends Element {
  static BASE_TYPE: IElementType = { name: "MASON", symbol: "Ma", class: Mason, color: 0x20ffff };
  ...
  exec(ew: EventWindow) {
    ...
    if (lastdir !== dir) {
      ...
      if (lastOuterBuildSite) {
        ew.origin.mutateSite(lastOuterBuildSite, new Atom(Wall.BASE_TYPE));
      }
    }

    //build the outer wall
    if (outerBuildSite) {
      if (outerBuildSite.atom.is(Res.BASE_TYPE) || outerBuildSite.atom.is(Empty.BASE_TYPE)) {
        ew.origin.mutateSite(outerBuildSite, new Atom(Wall.BASE_TYPE));
      }
    }

    //build the inner wall
    if (innerBuildSite) {
      if (innerBuildSite.atom.is(Res.BASE_TYPE) || innerBuildSite.atom.is(Empty.BASE_TYPE)) {
        ew.origin.mutateSite(innerBuildSite, new Atom(Wall.BASE_TYPE));
      }
    }

    //move to next site and leave another mason to help
    if (moveSite) {
      ew.origin.mutateSite(moveSite, new Atom(Mason.BASE_TYPE, [Mason.pathToString(this.path), this.curIndex + 1]));
    }

    if (lastSite) {
      // ew.origin.mutateSite(
      //   lastSite,
      //   new Atom(Mason.BASE_TYPE, [MasonElement.pathToString(this.path), this.curIndex - 1])
      // );
    }
...
ElementRegistry.registerType(Mason.BASE_TYPE);
