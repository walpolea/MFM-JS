...
export class RevolvingDoor extends Elem {
  static TYPE_DEF: IElementType = { name: "REVOLVINGDOOR", type: "Rd", class: RevolvingDoor, color: 0x4466ff };
  ...
  exec(ew: EventWindow) {
    ...
    if (
      (!ew.is(this.currentExits[0], Empty.TYPE_DEF) && ew.is(this.currentExits[1], Empty.TYPE_DEF)) ||
      (!ew.is(this.currentExits[1], Empty.TYPE_DEF) && ew.is(this.currentExits[0], Empty.TYPE_DEF))
    ) {
