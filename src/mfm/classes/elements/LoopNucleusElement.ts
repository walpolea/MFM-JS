...
export class LoopNucleusElement extends Elem {
  ...
  pCREATE_RES: number = 20;
  pCREATE_MEMBRANE: number = 2;
  ...
  exec(ew: EventWindow) {
    ...
    if (MFMUtils.oneIn(this.pCREATE_MEMBRANE)) {
      ew.origin.mutateSite(ew.getAdjacent4Way(ElementTypes.EMPTY), new Atom(ElementTypes.STICKYMEMBRANE, [ElementTypes.LOOPWORM, 0.65, 1]));
    }
