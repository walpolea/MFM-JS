...
class L {
  ...
  start(e, t) {
    const r = e.renderer, s = r.encoder, i = this._shader.gpuProgram;
    this._geometry = t, s.setGeometry(t), y.blendMode = "normal", r.pipeline.getPipeline(
      t,
      i,
      y
    );
  ...
  execute(e, t) {
    ...
    if (!t.bindGroup) {
      const u = t.textures;
      t.bindGroup = E(u.textures, u.count);
    }
...
class H {
  ...
  createGPUBuffer(e) {
    ...
    return e._updateID = 0, e.data && (se(e.data.buffer, t.getMappedRange()), t.unmap()), this._gpuBuffers[e.uid] = t, this._managedBuffers.push(e), t;
  }
  onBufferChange(e) {
   ...
   */
  onBufferDestroy(e) {
    this._gpuBuffers[e.uid].destroy(), this._managedBuffers.splice(this._managedBuffers.indexOf(e), 1), e.off("update", this.updateBuffer, this), e.off("change", this.onBufferChange, this), e.off("destroy", this.onBufferDestroy, this), this._gpuBuffers[e.uid] = null;
  }
  destroy() {
    this._managedBuffers.slice().forEach((e) => this.onBufferDestroy(e)), this._managedBuffers = null, this._gpuBuffers = null;
  }
}
H.extension = {
  type: [
...
class I {
  ...
  draw(e) {
    ...
    this.setPipelineFromGeometryProgramAndState(t, r.gpuProgram, s, i), this.setGeometry(t), this._setShaderBindGroups(r, c), t.indexBuffer ? this.renderPassEncoder.drawIndexed(
...
class j {
  constructor(e) {
    this._bindGroupHash = /* @__PURE__ */ Object.create(null), this._buffers = [], this._bindGroups = [], this._bufferResources = [], this._renderer = e, this._batchBuffer = new Ce({ minUniformOffsetAlignment: _ });
    const t = 256 / _;
    for (let r = 0; r < t; r++) {
      let s = S.UNIFORM | S.COPY_DST;
      r === 0 && (s |= S.COPY_SRC), this._buffers.push(new ie({
...
class N {
  constructor(e) {
    this._moduleCache = /* @__PURE__ */ Object.create(null), this._bufferLayoutsCache = /* @__PURE__ */ Object.create(null), this._pipeCache = /* @__PURE__ */ Object.create(null), this._pipeStateCaches = /* @__PURE__ */ Object.create(null), this._colorMask = 15, this._multisampleCount = 1, this._renderer = e;
  }
  contextChange(e) {
    this._gpu = e, this.setStencilMode(P.DISABLED), this._updatePipeHash();
  }
  ...
  _createPipeline(e, t, r, s) {
    const i = this._gpu.device, n = this._createVertexBufferLayouts(e), o = this._renderer.state.getColorTargets(r);
    o[0].writeMask = this._stencilMode === P.RENDERING_MASK_ADD ? 0 : this._colorMask;
  ...
  _generateBufferKey(e) {
    ...
    for (let n = 0; n < s.length; n++) {
      ...
      t[r++] = o.location, t[r++] = o.offset, t[r++] = o.format, t[r++] = o.stride;
    }
    const i = t.join("");
    return e._layoutKey = he(i, "geometry"), e._layoutKey;
  }
  _createVertexBufferLayouts(e) {
    ...
    return e.buffers.forEach((r) => {
      ...
      for (const n in e.attributes) {
        ...
        o.buffer === r && (s.arrayStride = o.stride, s.stepMode = o.instance ? "instance" : "vertex", i.push({
          shaderLocation: o.location,
          offset: o.offset,
          format: o.format
        }));
      }
      i.length && t.push(s);
    }), this._bufferLayoutsCache[e._layoutKey] = t, t;
  }
  _updatePipeHash() {
    const e = Ee(
      this._stencilMode,
...
}, ze = { blockBytes: 4, blockWidth: 1, blockHeight: 1 }, Le = {
  type: "compressed",
  upload(a, e, t) {
...
class Z {
  ...
  initSource(e) {
    ...
    const r = $[e.format] || { blockBytes: 4, blockWidth: 1, blockHeight: 1 }, s = Math.ceil(e.pixelWidth / r.blockWidth) * r.blockWidth, i = Math.ceil(e.pixelHeight / r.blockHeight) * r.blockHeight, n = {
      label: e.label,
  ...
  onUpdateMipmaps(e) {
    this._mipmapGenerator || (this._mipmapGenerator = new He(this._gpu.device));
    const t = this.getGpuSource(e);
  ...
  _createTextureBindGroup(e) {
    const t = e.source, r = t.uid;
    return this._bindGroupHash[r] = new A({
      0: t,
      1: t.style
  ...
  generateCanvas(e) {
    const t = this._renderer, r = t.gpu.device.createCommandEncoder(), s = ae.get().createCanvas();
...
class J {
  init() {
    const e = new pe({
      uTransformMatrix: { value: new ue(), type: "mat3x3<f32>" },
  ...
  execute(e, t) {
    ...
    for (let d = 0; d < u.instructionSize; d++) {
      ...
      if (s.groups[1] = g.bindGroup, !g.gpuBindGroup) {
        const B = g.textures;
        g.bindGroup = E(B.textures, B.count), g.gpuBindGroup = i.bindGroup.getBindGroup(
          g.bindGroup,
          s.gpuProgram,
          1
        );
...
class Q {
  ...
  execute(e, t) {
    ...
    if (!s)
      ...
      ce("Mesh shader has no gpuProgram", t.shader);
      return;
    }
...
const Fe = [
  ...
  W,
  O
], Ie = [...Te, j], We = [L, Q, J], ee = [], te = [], re = [];
G.handleByNamedList(l.WebGPUSystem, ee);
G.handleByNamedList(l.WebGPUPipes, te);
G.handleByNamedList(l.WebGPUPipesAdaptor, re);
G.add(...Fe, ...Ie, ...We);
class Ye extends de {
  constructor() {
    const e = {
